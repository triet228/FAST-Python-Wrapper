...
def build_input_json_structure(value):
    """Return a recursive structure map for FAST input JSON data.

    Inputs:
        value: Parsed JSON value from InputAircraft.json or Mission.json.

    Outputs:
        Nested dictionaries that preserve field names. Lists include their
        template length and the shape of the first item; scalar leaves show
        JSON-level types rather than Python implementation types.

    Assumptions:
        The committed input files are templates. Their contracts should enforce
        field names, container shapes, marker shapes, and broad JSON scalar
        types while leaving numeric int/float spelling free for users.
    """
...
def validate_json_structure_contract(data, contract, file_name, path=""):
    """Validate parsed input JSON against a committed structure contract.

    Inputs:
        data: Parsed input JSON subtree.
        contract: Structure subtree generated by build_input_json_structure().
    ...
    Assumptions:
        Lists may vary in length for user missions and aircraft matrices are
        already validated where length has FAST-specific meaning. The contract
        therefore enforces list item shape but not exact _length values.
    """
    ...
    if isinstance(contract, dict):
        ...
        for key in sorted(expected_keys):
            child_label = key if not path else f"{path}.{key}"
            validate_json_structure_contract(
                data[key],
                contract[key],
                file_name,
